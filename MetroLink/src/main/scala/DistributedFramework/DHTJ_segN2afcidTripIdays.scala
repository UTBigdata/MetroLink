...
//Indexing method：SSTT-Tree
object DHTJ_segN2afcidTripIdays extends Logging{
  def main(args: Array[String]): Unit = {
    ...
    val segN_afcidTripIdays: Dataset[(Int, Array[(Int, Array[(Int, Int)])])] = spark.read.parquet("/data/segN_afcidN2tripIdays").
      map(line => (line(0).toString.toInt,line(1).toString.split(";").map(v =>
        (v.split(":")(0).toInt,v.split(":")(1).split(",").map(_.split("#")).map(x=>(x(0).toInt,x(1).toInt))) //v.split(":")(1).split(",").map(_.split("#")).map(x=>(x(0).toInt,x(1).toInt)) v.split(":")(1)
      )))// (segN, Array[(afcidN, Array[(tripIndex,day)])])
    val segN2afcidTripIdays = sc.broadcast(segN_afcidTripIdays.collect().toMap)

    // 没有过滤全局相似性上界小于阈值的AFC轨迹
        ...
        apTrips.zipWithIndex.foreach { case (apTrip, index_ap) =>
          ...
          OD2segNs.value((apTrip._2, apTrip._4)).foreach { segN =>
            ...
            ovsegNs.foreach { ovsegN =>
              ...
              val afcidN2TripIdays_path = segN2afcidTripIdays.value.getOrElse(ovsegN,Array[(Int, Array[(Int,Int)])]())
              afcidN2TripIdays_path.foreach{afcidN_TripIdays =>
                val idxArr = afcidN_TripIdays._2.filter(_._2 == day).map(_._1)
                if(idxArr.size > 0){
                  val afcTrips: Array[(Int, Int, Int, Int)] = afcidN2Trips.getOrDefault(afcidN_TripIdays._1, Array[(Int, Int, Int, Int)]())
                  if ((afcTrips.size > apTrips.size && afcTrips.size <= apTrips.size / tau) || (afcTrips.size <= apTrips.size && (1.0 - tau + µW * tau) * afcTrips.size >= µW * tau * apTrips.size)) {//长度过滤

                    val idxArr1 = idxArr.filter(index_afc => {//时间过滤
                      val cur_afc = afcTrips(index_afc)
                      apTrip._1 > cur_afc._1 - 600 && apTrip._3 < cur_afc._3 + 600 //过滤出具有粗略时间覆盖关系的行程对
                    })
                    if(idxArr1.size > 0) {
                      val set = afcidN_rawOVTripPairs1.getOrElseUpdate(afcidN_TripIdays._1, new OpenHashSet[Long]())
    ...
    simpairs.filter(_._3 >=tau).toDF("apidN","afcidN","sim").write.mode("overwrite").parquet(s"/data/SSTT-Tree_simpair")
